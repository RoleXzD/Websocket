...
public class WSPacketFriendStartedTyping extends WSPacket {
    private String playerId;

    private boolean read;

    public WSPacketFriendStartedTyping() {}

    public WSPacketFriendStartedTyping(String playerId) {
    ...
    public void process(WebSocket conn, ServerHandler handler) throws IOException {
        if (this.read) {
            ...
            if (target != null) {
                handler.sendPacket(target.getConn(), new WSPacketFriendStartedTyping(sender.getPlayerId().toString()));
            }
        }
    }
}
